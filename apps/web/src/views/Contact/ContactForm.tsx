'use client';
import { useActionState } from 'react';
import { useEffect, useRef } from 'react';

import styles from './ContactForm.module.css';

import { Link } from 'ui/components/Link';
import { toast } from 'ui/components/Toaster';

import { Icon } from 'components/Icon';

import { ContactField } from './ContactField';
import { submitContactAction } from './_actions/submitContact';

import type { ContactFormState } from './_actions/submitContact';
import type { Dictionary } from 'i18n/dictionaries/es';
import type { Locale } from 'i18n/config';

const INITIAL_STATE: ContactFormState = {};

export interface ContactFormProps {
  copy: Dictionary['contact']['form'];
  /** Bound into the action so server-side validation answers in the visitor's language. */
  locale: Locale;
  /** Label + href for the privacy policy shown beneath the submit button. */
  privacy: { href: string; label: string };
}

export function ContactForm({ copy, locale, privacy }: ContactFormProps) {
  const [state, action, isPending] = useActionState(submitContactAction.bind(null, locale), INITIAL_STATE);
  const formRef = useRef<HTMLFormElement>(null);

  useEffect(() => {
    if (state.success) {
      toast.success(copy.success);
      formRef.current?.reset();
    }
  }, [state.success, copy.success]);

  return (
    // No `noValidate`: required / type=email / minLength are the browser's instant first
    // pass, in the user's language; the Zod schema server-side remains the source of truth.
    <form action={action} className={styles.form} ref={formRef}>
      <div className={styles.grid}>
        <ContactField
          defaultValue={state.values?.name}
          error={state.errors?.name}
          icon="user"
          label={copy.name}
          maxLength={120}
          minLength={2}
          name="name"
          required={true}
        />
        <ContactField
          autoComplete="email"
          defaultValue={state.values?.email}
          error={state.errors?.email}
          icon="mail"
          label={copy.email}
          name="email"
          required={true}
          type="email"
        />
        <ContactField
          autoComplete="tel"
          className={styles.wide}
          defaultValue={state.values?.phone}
          error={state.errors?.phone}
          icon="phone"
          label={copy.phone}
          maxLength={40}
          name="phone"
          type="tel"
        />

        <div className={`${styles.field} ${styles.wide}`}>
          <label className={styles.label} htmlFor="contact-message">
            <span className={styles.icon}>
              <Icon name="pencil" />
            </span>
            {copy.message}
            <span aria-hidden={true} className={styles.required}>
              *
            </span>
          </label>
          <textarea
            aria-describedby={state.errors?.message ? 'contact-message-error' : undefined}
            aria-invalid={state.errors?.message ? true : undefined}
            className={styles.textarea}
            defaultValue={state.values?.message}
            id="contact-message"
            maxLength={2000}
            minLength={10}
            name="message"
            required={true}
            rows={4}
          />
          {state.errors?.message ? (
            <p aria-live="polite" className={styles.error} id="contact-message-error">
              {state.errors.message}
            </p>
          ) : null}
        </div>
      </div>

      {/* Form-level failures (a dead database, an unexpected throw) are separate from
          per-field validation and get their own alert region. */}
      {state.errors?._form ? (
        <p className={styles.formError} role="alert">
          {state.errors._form}
        </p>
      ) : null}

      {/* Only the submit is disabled while pending — never the inputs, so a visitor can
          keep fixing a typo while the request is in flight. */}
      <button className={styles.submit} disabled={isPending} type="submit">
        {isPending ? copy.sending : copy.submit}
        <Icon className={styles.submitIcon} name="arrowRight" />
      </button>

      {/* GDPR asks that people be told who is collecting their data and why *at the point of
          collection* — a policy nobody is pointed to does not discharge that. */}
      <p className={styles.privacy}>
        {copy.privacyNotice}{' '}
        <Link href={privacy.href} inline={true}>
          {privacy.label}
        </Link>
        .
      </p>
    </form>
  );
}
